...
def n_queens(n,dp,dm,itr,sp,ruta, sampler=None):
    ...
    embedding_time = None
    if sp == 'DWaveSampler':
        sp_name = DWaveSampler()
        tick = time()
        sampler = EmbeddingComposite(sp_name)
        embedding_time = time()-tick
    else:
        sampler = sp_name = DWaveCliqueSampler()
    ...
    line = f'{n}   {d}   {itr}   {sp_name}   {psol}   {energy}   {n_vars}   {n_qb}   ' \
           f'{chain_strength}   {chain_break_method}   {py_time*10**3}   '
    line += f'{embedding_time*10**3}   ' if embedding_time else f'NA   '
