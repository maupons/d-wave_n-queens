...
def n_queens(n,dp,dm,itr,ruta, sampler=None):
    ...
    # sampler = EmbeddingComposite(DWaveSampler())
    # sp_name = str(sampler.child).split(".")[4].split()[0]
    sampler = DWaveCliqueSampler()
    sp_name = str(sampler).split(".")[4].split()[0]
    sampleset = sampler.sample(bqm, num_reads=itr, label=f'n {n}, time {start_time}')

    # qpu = DWaveSampler()
    # embedding = busclique.find_clique_embedding(bqm.variables, qpu.to_networkx_graph())
    # embedding = busclique.find_embedding(bqm.variables, qpu.to_networkx_graph())
    # embedding = busclique.find_embedding(bqm.variables, dnx.chimera_graph())
    #  dnx.chimera_graph()
    #  dnx.pegasus_graph()
    #  dnx.zephyr_graph()
    ...
    f4 = open(f"{ruta}timeQPU_test_embb.txt", "a")
    line = f'{n}   {d}   {itr}   {psol}   {n_vars}   {n_qb}   ' \
           f'{sp_name}   {chain_strength}   {chain_break_method}   ' \
           f'{py_time*10**3}   {done_time*10**3}   {energy}'
